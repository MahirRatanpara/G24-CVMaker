    ...
    .then((jsonRes) => {
      ...
      if (resumeData?.education && resumeData?.education.length > 0) {
        ...
        while (rowstobeupdated - tablerows.length > 0) {
          addEducation();
          tablerows = document.getElementById("education-table").tBodies[0]
            .rows;
  ...
  if (!resumeId) {
    ...
    fetch(`/api/resumeData/${resumeId}`, myInit)
      ...
      .then((jsonRes) => {
        ...
        const str =
          "alert alert-success alert-dismissible fade show container-md";
      ...
      .catch((err) => {
        ...
        const str =
          "alert alert-danger alert-dismissible fade show container-md";
        str.split(" ").forEach((c) => newDiv.classList.add(c));
